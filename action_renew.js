...
for (let i = 0; i < users.length; i++) {
        ...
        try {
            ...
            // 最多尝试 5 个不同的代理 IP，避免单个 IP 被墙导致死循环
            const maxIpAttempts = Math.min(proxyList.length > 0 ? proxyList.length : 1, 5);
            let loginSuccess = false;

            // --- 代理 IP 轮换重试循环 ---
            for (let ipAttempt = 1; ipAttempt <= maxIpAttempts; ipAttempt++) {
                if (proxyList.length > 0) {
                    const randomProxy = proxyList[Math.floor(Math.random() * proxyList.length)];
                    const [ip, port, user_p, pass_p] = randomProxy.split(':');
                    if (ip && port) {
                        const proxyVal = `http://${user_p}:${pass_p}@${ip}:${port}`;
                        process.env.HTTP_PROXY = proxyVal;
                        process.env.HTTPS_PROXY = proxyVal;
                        
                        // 动态更新 Playwright 代理配置或通过 Chrome 启动参数/CDP 生效
                        console.log(`[重试] 第 ${ipAttempt}/${maxIpAttempts} 次尝试切换代理 IP: ${ip}:${port}`);
                    }
                }
                ...
                try {
                    ...
                    if (loginUrl.includes('error=captcha') || loginBody.includes('Please complete captcha')) {
                        console.log(`   >> ⚠️ 当前 IP 触发了 Cloudflare 拦截 (?error=captcha)，自动更换下一个 IP 重试...`);
                        continue; // 直接跳到下一次循环，换下一个 IP
                    }
                    ...
                    if (await errorMsg.isVisible({ timeout: 2000 }).catch(() => false)) {
                        ...
                        break; // 账号密码错误不用换 IP，直接跳出重试
                    }

                    loginSuccess = true;
                    console.log(`[登录] ✅ 成功通过当前代理 IP 登录系统！`);
                    break; // 登录成功，跳出 IP 重试循环

                } catch (netErr) {
                    console.log(`[重试] 当前代理连接异常: ${netErr.message}，尝试换个 IP...`);
                }
            }

            if (!loginSuccess && runStatus !== 'login_failed') {
                runStatus = 'login_failed';
                blockMessage = 'Cloudflare captcha blocked all proxy attempts';
            }

            if (runStatus === 'login_failed') {
                const photoDir = await ensureScreenshotsDir();
            ...
            if (!dashboardReady) {
                ...
                if (/dashboard/i.test(bodyText) && /server/i.test(bodyText)) dashboardReady = true;
            }
            ...
            if (!dashboardReady) {
                console.log('login_failed: 未找到 dashboard 入口。');
                runStatus = 'login_failed';
                blockMessage = 'No dashboard entry found';
                const photoDir = await ensureScreenshotsDir();
